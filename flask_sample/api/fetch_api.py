...
@api.route('/fetch', methods=['GET']) #hg345 12-19-2023
@admin_permission.require(http_exception=403)  # only admin can fetch the movies
def fetch_media():
    load_dotenv()
    url = os.getenv("OTT_URL")
    api_key = os.getenv("RAPIDAPI_KEY")
    api_host = os.getenv("RAPIDAPI_HOST")
    querystring = {"start_year": "1970","end_year": "2022","min_imdb": "6","max_imdb": "7.8","genre": "horror","language": "english","page": "1" }
    ...
    for item in response.get('results', []):
        ...
        if image_urls:
            ...
            image_url = None
        imdb_id = item.get('imdbid')  # Check if the record with the same imdb_id already exists     
        record = DB.selectOne(  
            "SELECT * FROM movies WHERE imdb_id = %s", imdb_id)
        if record.row:
            continue
        DB.insertOne(  # Insert data into the MySQL database
